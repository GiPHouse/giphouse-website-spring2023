...
class MyClass:
    def add_and_remove_tags(self, account_id, destination_ou_id):
        ...
        client.tag_resource(ResourceId=account_id,
                            Tags=[{"Key": "CourseIteration", "Value": "Temporary"}])
    
        root_id = client.list_roots()["Roots"][0]["Id"]
        client.move_account(
            AccountId=account_id, SourceParentId=root_id,
            DestinationParentId=destination_ou_id
        )

        client.untag_resource(ResourceId=account_id,
                              TagKeys=["CourseIteration"])
